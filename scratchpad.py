...
class TextEditor:
  ...
  def __init__(self,root):
      ...
      for foldername in os.listdir("scratchpad_tools"):
          ...
          for filename in os.listdir(f"scratchpad_tools/{foldername}"):
  ...
  def gpt3(self,*args):
      ...
      prompt2 = f"WRITING STYLE:{style}" + prompt.replace(
          '<<TEXT>>', data
      ).replace('[2]', text2)
  ...
  def tool_open(self,x,y):
      ...
      print(f"***********************{filepath}/{y}/{x}")
      print(x)
      tool_file = f"{filepath}/{y}/{x}"
      with open(tool_file,"r") as infile:
          my_file = infile.read()
          left = "DESCRIPTION:"
          right = "END"
